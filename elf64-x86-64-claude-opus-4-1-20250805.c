...
elf_x86_64_rtype_to_howto (bfd *abfd, unsigned r_type)
{
  ...
  else if (r_type >= (unsigned int) R_X86_64_max)
    {
  ...
  if (i >= ARRAY_SIZE (x86_64_elf_howto_table))
    {
      _bfd_error_handler (_("%pB: relocation index out of bounds %#x"),
                          abfd, r_type);
      bfd_set_error (bfd_error_bad_value);
      return NULL;
    }

  if (x86_64_elf_howto_table[i].type != r_type)
    {
      _bfd_error_handler (_("%pB: relocation type mismatch %#x"),
                          abfd, r_type);
      bfd_set_error (bfd_error_bad_value);
      return NULL;
    }
                              ...
                              const char *r_name)
{
  ...
  if (!ABI_64_P (abfd) && strcasecmp (r_name, "R_X86_64_32") == 0)
    {
      unsigned int last_index = ARRAY_SIZE (x86_64_elf_howto_table) - 1;
      reloc_howto_type *reloc = &x86_64_elf_howto_table[last_index];
  ...
  for (i = 0; i < ARRAY_SIZE (x86_64_elf_howto_table); i++)
    {
      ...
      if (strcasecmp (x86_64_elf_howto_table[i].name, r_name) == 0)
        return &x86_64_elf_howto_table[i];
    }
			  ...
			  Elf_Internal_Rela *dst)
{
  ...
  if (r_type != howto->type && howto->type != R_X86_64_NONE)
    return false;
...
{
  struct elf_internal_linux_prpsinfo_offsets {
    int pid_offset;
    int program_offset;
    int command_offset;
  };

  static const struct elf_internal_linux_prpsinfo_offsets offsets[] = {
    { 12, 28, 44 },  /* 124: sizeof (struct elf_external_linux_prpsinfo32_ugid16) */
    { 12, 32, 48 },  /* 128: sizeof (struct elf_external_linux_prpsinfo32_ugid32) */
    { 24, 40, 56 }   /* 136: sizeof (struct elf_prpsinfo) on Linux/x86_64 */
  };

  const struct elf_internal_linux_prpsinfo_offsets *offset_ptr = NULL;

  if (note->descsz == 124)
    offset_ptr = &offsets[0];
  else if (note->descsz == 128)
    offset_ptr = &offsets[1];
  else if (note->descsz == 136)
    offset_ptr = &offsets[2];
  else
    return false;
  ...
  elf_tdata (abfd)->core->pid = bfd_get_32 (abfd, note->descdata + offset_ptr->pid_offset);
  elf_tdata (abfd)->core->program = _bfd_elfcore_strndup (abfd, note->descdata + offset_ptr->program_offset, 16);
  elf_tdata (abfd)->core->command = _bfd_elfcore_strndup (abfd, note->descdata + offset_ptr->command_offset, 80);
				 ...
				 const Elf_Internal_Rela *relend)
{
  ...
  switch (r_type)
    ...
    case R_X86_64_TLSGD:
    case R_X86_64_TLSLD:
      return check_tlsgd_tlsld_transition(abfd, sec, contents, symtab_hdr,
                                         sym_hashes, r_type, rel, relend,
                                         htab, offset);

    case R_X86_64_CODE_4_GOTTPOFF:
      return check_code4_gottpoff_transition(abfd, sec, contents, offset);

    case R_X86_64_CODE_5_GOTTPOFF:
      return check_code5_gottpoff_transition(abfd, sec, contents, offset);

    case R_X86_64_CODE_6_GOTTPOFF:
      return check_code6_gottpoff_transition(abfd, sec, contents, offset);

    case R_X86_64_GOTTPOFF:
      return check_gottpoff_transition(abfd, sec, contents, offset);

    case R_X86_64_CODE_4_GOTPC32_TLSDESC:
      return check_code4_tlsdesc_transition(sec, contents, offset);

    case R_X86_64_GOTPC32_TLSDESC:
      return check_gotpc32_tlsdesc_transition(abfd, sec, contents, offset);
...
static enum elf_x86_tls_error_type
check_tlsgd_tlsld_transition(bfd *abfd, asection *sec, bfd_byte *contents,
                            Elf_Internal_Shdr *symtab_hdr,
                            struct elf_link_hash_entry **sym_hashes,
                            unsigned int r_type,
                            const Elf_Internal_Rela *rel,
                            ...
                            bfd_vma offset)
{
  ...
  if (r_type == R_X86_64_TLSGD)
    {
      ...
      if (!validate_tlsgd_call_sequence(call))
        {
          if (!validate_tlsgd_largepic_sequence(abfd, sec, call, leaq, offset))
            return elf_x86_tls_error_yes;
          largepic = true;
        }
      else if (!validate_tlsgd_prefix(abfd, contents, leaq, offset))
        {
          return elf_x86_tls_error_yes;
        }
  ...
  else
    {
      ...
      call = contents + offset + 4;
      if (!validate_tlsld_call_sequence(call))
        {
          if (!validate_tlsld_largepic_sequence(abfd, sec, call, offset))
            return elf_x86_tls_error_yes;
          largepic = true;
        }
  ...
  return validate_relocation_type(rel, largepic, indirect_call);
}

static bool
validate_tlsgd_call_sequence(bfd_byte *call)
{
  return call[0] == 0x66 &&
...
static bool
validate_tlsgd_largepic_sequence(bfd *abfd, asection *sec, bfd_byte *call,
                                const unsigned char *leaq, bfd_vma offset)
{
  return ABI_64_P(abfd) &&
         (offset + 19) <= sec->size &&
         offset >= 3 &&
...
static bool
validate_tlsgd_prefix(bfd *abfd, bfd_byte *contents,
                     const unsigned char *leaq, bfd_vma offset)
{
  if (ABI_64_P(abfd))
    return offset >= 4 && memcmp(contents + offset - 4, leaq, 4) == 0;
  else
    return offset >= 3 && memcmp(contents + offset - 3, leaq + 1, 3) == 0;
}

static bool
validate_tlsld_call_sequence(bfd_byte *call)
{
  return call[0] == 0xe8 ||
...
static bool
validate_tlsld_largepic_sequence(bfd *abfd, asection *sec,
                                bfd_byte *call, bfd_vma offset)
{
  return ABI_64_P(abfd) &&
         (offset + 19) <= sec->size &&
         memcmp(call, "\x48\xb8", 2) == 0 &&
...
static enum elf_x86_tls_error_type
validate_relocation_type(const Elf_Internal_Rela *rel,
                        bool largepic, bool indirect_call)
{
  unsigned int r_type = (ELF32_R_TYPE(rel[1].r_info) &
...
static enum elf_x86_tls_error_type
check_code4_gottpoff_transition(bfd *abfd, asection *sec,
                               bfd_byte *contents, bfd_vma offset)
{
  ...
  if (!ABI_64_P(abfd) &&
    ...
    return check_gottpoff_modrm(contents, offset);

  if (contents[offset - 4] != 0xd5)
    return elf_x86_tls_error_yes;

  return check_gottpoff_instruction(abfd, contents, offset);
}

static enum elf_x86_tls_error_type
check_code5_gottpoff_transition(bfd *abfd, asection *sec,
                               bfd_byte *contents, bfd_vma offset)
{
...
static enum elf_x86_tls_error_type
check_code6_gottpoff_transition(bfd *abfd, asection *sec,
                               bfd_byte *contents, bfd_vma offset)
{
  unsigned int val;

  if (offset < 6 || (offset + 4) > sec->size ||
...
static enum elf_x86_tls_error_type
check_gottpoff_transition(bfd *abfd, asection *sec,
                         bfd_byte *contents, bfd_vma offset)
{
  unsigned int val;
...
static enum elf_x86_tls_error_type
check_gottpoff_instruction(bfd *abfd, bfd_byte *contents, bfd_vma offset)
{
  unsigned int val = bfd_get_8(abfd, contents + offset - 2);
...
static enum elf_x86_tls_error_type
check_gottpoff_modrm(bfd_byte *contents, bfd_vma offset)
{
  unsigned int val = bfd_get_8(NULL, contents + offset - 1);
  return ((val & 0xc7) == 5) ?
         elf_x86_tls_error_none : elf_x86_tls_error_yes;
}

static enum elf_x86_tls_error_type
check_code4_tlsdesc_transition(asection *sec, bfd_byte *contents,
                              bfd_vma offset)
{
  if (offset < 4 || (offset + 4) > sec->size ||
  ...
  return check_tlsdesc_instruction(contents, offset);
}

static enum elf_x86_tls_error_type
check_gotpc32_tlsdesc_transition(bfd *abfd, asection *sec,
                                bfd_byte *contents, bfd_vma offset)
{
  unsigned int val;
...
static enum elf_x86_tls_error_type
check_tlsdesc_instruction(bfd_byte *contents, bfd_vma offset)
{
  unsigned int val;

  if (bfd_get_8(NULL, contents + offset - 2) != 0x8d)
    return elf_x86_tls_error_lea;

  val = bfd_get_8(NULL, contents + offset - 1);
			   ...
			   bool from_relocate_section)
{
  ...
  switch (from_type)
    {
    case R_X86_64_TLSDESC_CALL:
      if (!elf_x86_64_validate_tlsdesc_call(abfd, sec, contents, rel, info, symtab_hdr, h, sym))
        return false;
    ...
    case R_X86_64_CODE_5_GOTTPOFF:
    case R_X86_64_CODE_6_GOTTPOFF:
      to_type = elf_x86_64_determine_tls_transition_type(info, h, to_type, tls_type, from_relocate_section, from_type, &check);
      break;
  ...
  if (elf_x86_64_is_transition_unnecessary(from_type, to_type))
    return true;

  if (check && !elf_x86_64_perform_transition_check(abfd, info, sec, contents, symtab_hdr, sym_hashes, from_type, to_type, rel, relend, h, sym))
    return false;
...
static bool
elf_x86_64_validate_tlsdesc_call(bfd *abfd, asection *sec, bfd_byte *contents,
                                  const Elf_Internal_Rela *rel,
                                  struct bfd_link_info *info,
                                  Elf_Internal_Shdr *symtab_hdr,
                                  struct elf_link_hash_entry *h,
                                  Elf_Internal_Sym *sym)
{
  ...
  if (call[prefix] != 0xff || call[1 + prefix] != 0x10)
    {
      _bfd_x86_elf_link_report_tls_transition_error
        (info, abfd, sec, symtab_hdr, h, sym, rel,
         "R_X86_64_TLSDESC_CALL", NULL,
...
static unsigned int
elf_x86_64_determine_tls_transition_type(struct bfd_link_info *info,
                                          struct elf_link_hash_entry *h,
                                          ...
                                          bool *check)
{
  ...
  *check = (new_to_type != to_type &&
            (from_type == to_type ||
             elf_x86_64_is_code_gottpoff_transition(from_type, to_type)));

  return new_to_type;
}

static bool
elf_x86_64_is_code_gottpoff_transition(unsigned int from_type, unsigned int to_type)
{
  return ((from_type == R_X86_64_CODE_4_GOTTPOFF ||
...
static bool
elf_x86_64_is_transition_unnecessary(unsigned int from_type, unsigned int to_type)
{
...
static bool
elf_x86_64_perform_transition_check(bfd *abfd, struct bfd_link_info *info,
                                     asection *sec, bfd_byte *contents,
                                     Elf_Internal_Shdr *symtab_hdr,
                                     struct elf_link_hash_entry **sym_hashes,
                                     ...
                                     Elf_Internal_Sym *sym)
{
  ...
  _bfd_x86_elf_link_report_tls_transition_error(
      info, abfd, sec, symtab_hdr, h, sym, rel, from->name, to->name, tls_error);
		     ...
		     reloc_howto_type *howto)
{
  ...
  if (h)
    {
      ...
      visibility_str = get_visibility_string(h);
      
      if (ELF_ST_VISIBILITY (h->other) == STV_DEFAULT && 
          !((struct elf_x86_link_hash_entry *) h)->def_protected)
        {
          recompile_str = NULL;
        }
  ...
  if (bfd_link_dll (info))
    {
      ...
      if (!recompile_str)
        {
          recompile_str = _("; recompile with -fPIC");
        }
    }
  else if (bfd_link_pie (info))
    {
      object_type = _("a PIE object");
      if (!recompile_str)
        {
          recompile_str = _("; recompile with -fPIE");
        }
    }
  else
    {
      object_type = _("a PDE object");
      if (!recompile_str)
        {
          recompile_str = _("; recompile with -fPIE");
        }
  ...
  _bfd_error_handler (_("%pB: relocation %s against %s%s`%s' can "
			"not be used when making %s%s"),
		      input_bfd, howto->name, undefined_str, visibility_str, 
		      symbol_name, object_type, recompile_str);
  bfd_set_error (bfd_error_bad_value);
...
static const char *
get_visibility_string(struct elf_link_hash_entry *h)
{
...
static unsigned int evex_move_r_to_b(unsigned int byte1, bool copy)
{
    ...
    unsigned int r3_bit = (byte1 & R3_MASK) >> 2;
    unsigned int r4_bit_inverted = (~byte1 & R4_MASK) >> 1;
    
    unsigned int result = byte1;
    result &= ~B3_MASK;
    result |= r3_bit;
    result &= ~B4_MASK;
    result |= r4_bit_inverted;
                               ...
                               struct bfd_link_info *link_info)
{
  ...
  if (!validate_instruction(opcode, modrm, relocx, &is_branch))
    return true;
  ...
  if (h == NULL)
  {
    if (!process_local_symbol(abfd, htab, r_symndx, &isym, &tsec, 
                              &abs_symbol, &abs_relocation, &local_ref))
      return true;
  }
  else
  {
    if (!process_global_symbol(h, link_info, opcode, relocx, no_overflow,
                               is_branch, to_reloc_pc32, is_pic, htab,
                               &tsec, &abs_symbol, &abs_relocation, 
                               &local_ref, &isym))
      return true;
  }
...
convert:
  if (!compute_relocation(h, isym, link_info, irel, &tsec, 
                          &relocation, &raddend))
    return false;

  if (is_branch)
  {
    if (!convert_branch_instruction(abfd, contents, irel, h, htab,
                                    relocation, raddend, modrm, &r_type))
      return true;
  }
  else if (r_type == R_X86_64_CODE_6_GOTPCRELX && opcode != 0x8b)
  {
    if (!convert_evex_instruction(abfd, contents, irel, roff, opcode,
                                  modrm, evex, to_reloc_pc32, relocation,
                                  abs_relocation, abs_symbol, &r_type))
      return true;
  }
  else
  {
    if (!convert_regular_instruction(abfd, contents, irel, roff, opcode,
                                     modrm, r_type, abs_symbol, local_ref,
                                     relocx, to_reloc_pc32, link_info,
                                     relocation, raddend, abs_relocation,
                                     htab, &r_type))
      return true;
  }
...
static bool
validate_relocation_type(unsigned int r_type, bfd_vma roff, bfd *abfd,
                         bfd_byte *contents, unsigned char *evex, bool *relocx)
{
  unsigned int opcode;
  ...
  switch (r_type)
  {
    ...
    case R_X86_64_CODE_6_GOTPCRELX:
      ...
      if ((evex[0] & 7) != 4 || (evex[1] & 3) != 0 || (evex[2] & 0xe0) != 0)
        return false;
      *relocx = true;
      break;
  }
...
static bool
validate_instruction(unsigned int opcode, unsigned int modrm, 
                    bool relocx, bool *is_branch)
{
  if (opcode == 0xff)
  {
    switch (modrm & 0x38)
...
static bool
process_local_symbol(bfd *abfd, struct elf_x86_link_hash_table *htab,
                     unsigned int r_symndx, Elf_Internal_Sym **isym,
                     asection **tsec, bool *abs_symbol,
                     bfd_vma *abs_relocation, bool *local_ref)
{
...
static bool
process_global_symbol(struct elf_link_hash_entry *h, 
                     struct bfd_link_info *link_info,
                     unsigned int opcode, bool relocx, bool no_overflow,
                     bool is_branch, bool to_reloc_pc32, bool is_pic,
                     struct elf_x86_link_hash_table *htab,
                     asection **tsec, bool *abs_symbol,
                     bfd_vma *abs_relocation, bool *local_ref,
                     Elf_Internal_Sym **isym)
...
static bool
compute_relocation(struct elf_link_hash_entry *h, Elf_Internal_Sym *isym,
                   struct bfd_link_info *link_info, Elf_Internal_Rela *irel,
                   asection **tsec, bfd_vma *relocation, bfd_signed_vma *raddend)
{
  if (h == NULL)
  {
  ...
  {
    *relocation = h->root.u.def.value + (*tsec)->output_section->vma +
                  (*tsec)->output_offset;
  }
  else
  {
...
static bool
convert_branch_instruction(bfd *abfd, bfd_byte *contents, Elf_Internal_Rela *irel,
                           struct elf_link_hash_entry *h,
                           struct elf_x86_link_hash_table *htab,
                           bfd_vma relocation, bfd_signed_vma raddend,
                           unsigned int modrm, unsigned int *r_type)
{
  unsigned int nop;
  unsigned int disp;
  bfd_vma nop_offset;
...
static bool
convert_evex_instruction(bfd *abfd, bfd_byte *contents, Elf_Internal_Rela *irel,
                         bfd_vma roff, unsigned int opcode, unsigned int modrm,
                         unsigned char *evex, bool to_reloc_pc32,
                         bfd_vma relocation, bfd_vma abs_relocation,
                         bool abs_symbol, unsigned int *r_type)
{
  bool move_v_r = false;
  reloc_howto_type *howto;
  bfd_reloc_status_type r;
...
static bool
convert_regular_instruction(bfd *abfd, bfd_byte *contents, Elf_Internal_Rela *irel,
                            bfd_vma roff, unsigned int opcode, unsigned int modrm,
                            unsigned int r_type, bool abs_symbol, bool local_ref,
                            bool relocx, bool to_reloc_pc32,
                            struct bfd_link_info *link_info, bfd_vma relocation,
                            bfd_signed_vma raddend, bfd_vma abs_relocation,
                            struct elf_x86_link_hash_table *htab,
                            unsigned int *r_type_p)
{
  unsigned int rex = 0;
  ...
  if (!extract_rex_info(r_type, roff, abfd, contents, &rex, &rex2,
                       &rex_mask, &rex2_mask, &rex_w, &movrs))
    return true;
  ...
  if (opcode == 0x8b)
  {
    ...
    {
      opcode = 0x8d;
      *r_type_p = R_X86_64_PC32;
      howto = &x86_64_elf_howto_table[*r_type_p];
      r = _bfd_final_link_relocate (howto, abfd, input_section,
                                    contents, irel->r_offset,
                                    relocation, raddend);
      if (r == bfd_reloc_overflow)
    ...
    else
    {
      ...
      {
        *r_type_p = R_X86_64_32S;
      }
      else
      {
        *r_type_p = R_X86_64_32;
        rex_mask |= REX_W;
        rex2_mask |= REX_W;
      }
      
      if (!perform_final_relocation(abfd, input_section, contents, irel,
                                    *r_type_p, relocation, abs_relocation))
        return false;
      
      bfd_put_8 (abfd, modrm, contents + roff - 1);
      update_rex_bytes(abfd, contents, roff, rex, rex2, rex_mask, rex2_mask);
      irel->r_addend = 0;
    }
  }
  else
  {
    ...
    if (!convert_special_instruction(abfd, contents, irel, roff, &opcode,
                                     &modrm, rex, rex2, &rex_mask, &rex2_mask,
                                     rex_w, r_type_p))
      return false;
...
static bool
extract_rex_info(unsigned int r_type, bfd_vma roff, bfd *abfd,
                 bfd_byte *contents, unsigned int *rex, unsigned int *rex2,
                 unsigned int *rex_mask, unsigned int *rex2_mask,
                 bool *rex_w, unsigned int *movrs)
{
...
static bool
perform_final_relocation(bfd *abfd, asection *input_section, bfd_byte *contents,
                        Elf_Internal_Rela *irel, unsigned int r_type,
                        bfd_vma relocation, bfd_vma abs_relocation)
{
...
static void
update_rex_bytes(bfd *abfd, bfd_byte *contents, bfd_vma roff,
...
static void
apply_movrs_fixup(bfd *abfd, bfd_byte *contents, bfd_vma roff,
...
static bool
convert_special_instruction(bfd *abfd, bfd_byte *contents, Elf_Internal_Rela *irel,
                            bfd_vma roff, unsigned int *opcode, unsigned int *modrm,
                            unsigned int rex, unsigned int rex2,
                            unsigned int *rex_mask, unsigned int *rex2_mask,
                            bool rex_w, unsigned int *r_type_p)
{
...
process_relocation(bfd *abfd, struct bfd_link_info *info, asection *sec,
                   ...
                   Elf_Internal_Shdr *symtab_hdr, struct elf_link_hash_entry **sym_hashes,
                   bfd_byte *contents, bool *converted)
{
  unsigned int r_type;
  unsigned int r_symndx;
  struct elf_link_hash_entry *h;
  ...
  if (!validate_relocation(abfd, sec, rel, r_symndx, r_type, symtab_hdr, &howto))
    return false;

  if (!get_symbol_info(abfd, htab, symtab_hdr, sym_hashes, r_symndx, rel, 
                       &h, &isym))
    return false;

  if (!check_x32_relocation(abfd, symtab_hdr, isym, h, r_type))
    return false;
  ...
  if (!handle_gotpcrel_conversion(abfd, sec, contents, info, h, rel, 
                                  &r_type, &converted_reloc, converted))
    return false;
  ...
  return handle_relocation_type(abfd, info, sec, rel, htab, symtab_hdr,
                                r_type, r_symndx, h, eh, isym, 
                                no_dynreloc, converted_reloc);
}

static bool
validate_relocation(bfd *abfd, asection *sec, const Elf_Internal_Rela *rel,
...
static bool
get_symbol_info(bfd *abfd, struct elf_x86_link_hash_table *htab,
                Elf_Internal_Shdr *symtab_hdr, struct elf_link_hash_entry **sym_hashes,
                unsigned int r_symndx, const Elf_Internal_Rela *rel,
...
static bool
check_x32_relocation(bfd *abfd, Elf_Internal_Shdr *symtab_hdr,
                     Elf_Internal_Sym *isym, struct elf_link_hash_entry *h,
                     unsigned int r_type)
{
  const char *name;
...
static bool
handle_gotpcrel_conversion(bfd *abfd, asection *sec, bfd_byte *contents,
                           struct bfd_link_info *info, struct elf_link_hash_entry *h,
                           const Elf_Internal_Rela *rel, unsigned int *r_type,
                           bool *converted_reloc, bool *converted)
{
  if (((*r_type == R_X86_64_GOTPCREL) ||
      ...
      (h == NULL || h->type != STT_GNU_IFUNC))
    {
      ...
      if (!elf_x86_64_convert_load_reloc (abfd, sec, contents, r_type, 
                                          irel, h, converted_reloc, info))
        return false;
...
static bool
handle_relocation_type(bfd *abfd, struct bfd_link_info *info, asection *sec,
                       const Elf_Internal_Rela *rel, struct elf_x86_link_hash_table *htab,
                       ...
                       bool no_dynreloc, bool converted_reloc)
{
  switch (r_type)
    {
    case R_X86_64_TLSLD:
    ...
    case R_X86_64_TLSDESC_CALL:
      ...
      return handle_got_relocation(abfd, info, htab, symtab_hdr, r_type, 
                                  r_symndx, h, eh, isym);
    ...
    case R_X86_64_PLT32:
      return handle_plt32_relocation(h, eh);
    ...
    case R_X86_64_SIZE64:
      return handle_size_relocation(abfd, info, sec, htab, r_type, r_symndx,
                                    h, eh, isym, no_dynreloc, true);

    case R_X86_64_32:
      if (!ABI_64_P (abfd))
        return handle_pointer_relocation(abfd, info, sec, htab, symtab_hdr,
                                        r_type, r_symndx, h, eh, isym,
                                        no_dynreloc, converted_reloc);
    case R_X86_64_8:
    case R_X86_64_16:
    case R_X86_64_32S:
      return handle_small_relocation(abfd, info, sec, htab, symtab_hdr,
                                     r_type, r_symndx, h, eh, isym,
                                     no_dynreloc, converted_reloc);
    ...
    case R_X86_64_CODE_6_GOTPC32_TLSDESC:
      return handle_unsupported_relocation(abfd, symtab_hdr, r_type, h, isym);
                      ...
                      struct elf_x86_link_hash_entry *eh, Elf_Internal_Sym *isym)
{
  ...
  tls_type = get_tls_type(r_type, h, isym);
...
static int
get_tls_type(unsigned int r_type, struct elf_link_hash_entry *h,
             Elf_Internal_Sym *isym)
{
  switch (r_type)
    {
...
static bool
handle_plt32_relocation(struct elf_link_hash_entry *h,
                        struct elf_x86_link_hash_entry *eh)
{
  if (h == NULL)
    return true;
...
static bool
handle_small_relocation(bfd *abfd, struct bfd_link_info *info, asection *sec,
                       struct elf_x86_link_hash_table *htab,
                       Elf_Internal_Shdr *symtab_hdr, unsigned int r_type,
                       unsigned int r_symndx, struct elf_link_hash_entry *h,
                       struct elf_x86_link_hash_entry *eh, Elf_Internal_Sym *isym,
                       bool no_dynreloc, bool converted_reloc)
{
  if (!htab->params->no_reloc_overflow_check && !converted_reloc &&
      (bfd_link_pic (info) ||
       (bfd_link_executable (info) && h != NULL && !h->def_regular &&
        h->def_dynamic && (sec->flags & SEC_READONLY) == 0)))
    {
      elf_x86_64_need_pic (info, abfd, sec, h, symtab_hdr, isym,
                          &x86_64_elf_howto_table[r_type]);
      return false;
...
static bool
handle_pointer_relocation(bfd *abfd, struct bfd_link_info *info, asection *sec,
                          struct elf_x86_link_hash_table *htab,
                          Elf_Internal_Shdr *symtab_hdr, unsigned int r_type,
                          unsigned int r_symndx, struct elf_link_hash_entry *h,
                          struct elf_x86_link_hash_entry *eh, Elf_Internal_Sym *isym,
                          bool no_dynreloc, bool converted_reloc)
{
  ...
  if (h != NULL && (bfd_link_executable (info) || h->type == STT_GNU_IFUNC))
    {
      if (!process_function_relocation(abfd, info, sec, htab, r_type, h, eh))
        return false;
    }

  return handle_size_relocation(abfd, info, sec, htab, r_type, r_symndx,
                                h, eh, isym, no_dynreloc, false);
}

static bool
process_function_relocation(bfd *abfd, struct bfd_link_info *info, asection *sec,
                            struct elf_x86_link_hash_table *htab, unsigned int r_type,
                            struct elf_link_hash_entry *h, struct elf_x86_link_hash_entry *eh)
{
  ...
  if (r_type == R_X86_64_PC32)
      ...
      if ((sec->flags & SEC_CODE) == 0)
        {
          ...
          if (bfd_link_pie (info) && h->type == STT_FUNC &&
  ...
  else if (r_type != R_X86_64_PC64)
    {
      ...
      if (!func_pointer_ref ||
          (bfd_link_pde (info) && h->type == STT_GNU_IFUNC))
        h->pointer_equality_needed = 1;
    }
  ...
  if (!func_pointer_ref)
    {
      ...
      if (htab->elf.target_os != is_solaris && h->pointer_equality_needed &&
          h->type == STT_FUNC && eh->def_protected &&
...
static bool
handle_size_relocation(bfd *abfd, struct bfd_link_info *info, asection *sec,
                       struct elf_x86_link_hash_table *htab, unsigned int r_type,
                       unsigned int r_symndx, struct elf_link_hash_entry *h,
                       struct elf_x86_link_hash_entry *eh, Elf_Internal_Sym *isym,
                       bool no_dynreloc, bool size_reloc)
{
  struct elf_dyn_relocs *p;
  struct elf_dyn_relocs **head;

  if (!no_dynreloc &&
      NEED_DYNAMIC_RELOCATION_P (true, info, true, h, sec, r_type,
                                 htab->pointer_r_type))
    {
      if (h != NULL)
        head = &h->dyn_relocs;
...
static bool
handle_unsupported_relocation(bfd *abfd, Elf_Internal_Shdr *symtab_hdr,
  ...
  for (; rel < relend; wrel++, rel++)
    {
      if (!process_relocation(output_bfd, info, input_bfd, input_section,
                              contents, rel, wrel, &status, htab, symtab_hdr,
                              sym_hashes, local_got_offsets, local_tlsdesc_gotents,
                              plt_entry_size, relend, local_syms, local_sections))
        return false;
    }

  update_relocation_counts(input_section, rel, wrel);
  return status;
}

static bool
process_relocation(bfd *output_bfd, struct bfd_link_info *info, bfd *input_bfd,
                   asection *input_section, bfd_byte *contents,
                   Elf_Internal_Rela *rel, Elf_Internal_Rela *wrel,
                   bool *status, struct elf_x86_link_hash_table *htab,
                   Elf_Internal_Shdr *symtab_hdr,
                   struct elf_link_hash_entry **sym_hashes,
                   bfd_vma *local_got_offsets, bfd_vma *local_tlsdesc_gotents,
                   unsigned int plt_entry_size, Elf_Internal_Rela *relend,
                   Elf_Internal_Sym *local_syms, asection **local_sections)
{
  unsigned int r_type = ELF32_R_TYPE (rel->r_info);
  unsigned long r_symndx = htab->r_sym (rel->r_info);
  
  if (handle_vtable_reloc(r_type, rel, wrel))
    return true;
    
  if (handle_converted_reloc(&r_type, r_symndx, rel, htab))
    return true;
  ...
  struct reloc_info rinfo = {0};
  if (!get_symbol_info(&rinfo, r_symndx, symtab_hdr, local_syms, local_sections,
                       sym_hashes, rel, htab, info, input_bfd))
    return false;

  if (handle_discarded_section(&rinfo, howto, input_bfd, input_section,
                               contents, rel, wrel, info))
    return true;
  ...
  handle_x32_relocs(&r_type, output_bfd, contents, rel);

  if (rinfo.h && rinfo.h->type == STT_GNU_IFUNC && rinfo.h->def_regular)
    return handle_ifunc_reloc(output_bfd, info, input_bfd, input_section,
                             contents, rel, wrel, &rinfo, r_type, howto,
                             htab, plt_entry_size, status);

  return handle_regular_reloc(output_bfd, info, input_bfd, input_section,
                              contents, rel, wrel, &rinfo, r_type, howto,
                              htab, local_got_offsets, local_tlsdesc_gotents,
                              plt_entry_size, relend, symtab_hdr, sym_hashes,
                              status);
}

static bool
handle_vtable_reloc(unsigned int r_type, Elf_Internal_Rela *rel,
                    Elf_Internal_Rela *wrel)
{
...
static bool  
handle_converted_reloc(unsigned int *r_type, unsigned long r_symndx,
                       Elf_Internal_Rela *rel,
                       struct elf_x86_link_hash_table *htab)
{
  bool converted = (*r_type & R_X86_64_converted_reloc_bit) != 0;
  if (converted)
    {
      *r_type &= ~R_X86_64_converted_reloc_bit;
...
static bool
get_symbol_info(struct reloc_info *rinfo, unsigned long r_symndx,
                ...
                struct bfd_link_info *info, bfd *input_bfd)
{
  if (r_symndx < symtab_hdr->sh_info)
    {
      rinfo->sym = local_syms + r_symndx;
      rinfo->sec = local_sections[r_symndx];
      rinfo->relocation = _bfd_elf_rela_local_sym (input_bfd, rinfo->sym,
                                                   &rinfo->sec, rel);
      rinfo->st_size = rinfo->sym->st_size;

      if (!bfd_link_relocatable (info) &&
          ELF_ST_TYPE (rinfo->sym->st_info) == STT_GNU_IFUNC)
        {
          rinfo->h = _bfd_elf_x86_get_local_sym_hash (htab, input_bfd, rel, false);
          if (!rinfo->h)
            abort ();
          rinfo->h->root.u.def.value = rinfo->sym->st_value;
          rinfo->h->root.u.def.section = rinfo->sec;
        }
    }
  else
    {
      ...
      RELOC_FOR_GLOBAL_SYMBOL (info, input_bfd, input_section, rel,
                              ...
                              rinfo->h, rinfo->sec, rinfo->relocation,
                              rinfo->unresolved_reloc, warned, ignored);
      rinfo->st_size = rinfo->h->size;
    }

  if (rinfo->h)
    rinfo->eh = (struct elf_x86_link_hash_entry *) rinfo->h;
    
  return true;
}

static bool
handle_discarded_section(struct reloc_info *rinfo, reloc_howto_type *howto,
                         bfd *input_bfd, asection *input_section,
                         bfd_byte *contents, Elf_Internal_Rela *rel,
                         Elf_Internal_Rela *wrel, struct bfd_link_info *info)
{
  if (rinfo->sec && discarded_section (rinfo->sec))
    {
...
static void
handle_x32_relocs(unsigned int *r_type, bfd *output_bfd,
                  bfd_byte *contents, Elf_Internal_Rela *rel)
{
...
static bool
handle_ifunc_reloc(bfd *output_bfd, struct bfd_link_info *info, bfd *input_bfd,
                   asection *input_section, bfd_byte *contents,
                   Elf_Internal_Rela *rel, Elf_Internal_Rela *wrel,
                   struct reloc_info *rinfo, unsigned int r_type,
                   reloc_howto_type *howto, struct elf_x86_link_hash_table *htab,
                   unsigned int plt_entry_size, bool *status)
{
  if ((input_section->flags & SEC_ALLOC) == 0)
    {
      if (elf_section_type (input_section) == SHT_NOTE)
        return handle_regular_reloc(output_bfd, info, input_bfd, input_section,
                                   contents, rel, wrel, rinfo, r_type, howto,
                                   htab, NULL, NULL, plt_entry_size, NULL, NULL,
                                   NULL, status);
  ...
  switch (r_type)
    ...
    case R_X86_64_CODE_6_GOTPCRELX:
    case R_X86_64_GOTPCREL64:
      return handle_ifunc_got_reloc(output_bfd, info, input_bfd, input_section,
                                    contents, rel, wrel, rinfo, r_type, howto,
                                    htab, plt_entry_size, status);
    ...
    case R_X86_64_32:
      if (!ABI_64_P (output_bfd))
        return handle_ifunc_pointer(output_bfd, info, input_bfd, input_section,
                                   contents, rel, rinfo, howto, htab, status);
      break;
    ...
    default:
      return handle_bad_ifunc_reloc(input_bfd, rinfo, howto, symtab_hdr,
                                   rinfo->sym);
    }

  return handle_ifunc_plt(output_bfd, info, input_bfd, input_section,
                          contents, rel, wrel, rinfo, r_type, howto, htab,
                          plt_entry_size, status);
}

static bool
handle_ifunc_got_reloc(bfd *output_bfd, struct bfd_link_info *info,
                       bfd *input_bfd, asection *input_section,
                       bfd_byte *contents, Elf_Internal_Rela *rel,
                       Elf_Internal_Rela *wrel, struct reloc_info *rinfo,
                       unsigned int r_type, reloc_howto_type *howto,
                       struct elf_x86_link_hash_table *htab,
                       unsigned int plt_entry_size, bool *status)
{
  asection *base_got = htab->elf.sgot;
  bfd_vma off = rinfo->h->got.offset;
  ...
  if (off == (bfd_vma) -1)
    {
      if (rinfo->h->plt.offset == (bfd_vma) -1)
        abort ();

      bfd_vma plt_index;
      if (htab->elf.splt)
        {
          plt_index = (rinfo->h->plt.offset / plt_entry_size - htab->plt.has_plt0);
          off = (plt_index + 3) * GOT_ENTRY_SIZE;
          base_got = htab->elf.sgotplt;
        }
      else
        {
          plt_index = rinfo->h->plt.offset / plt_entry_size;
          off = plt_index * GOT_ENTRY_SIZE;
          base_got = htab->elf.igotplt;
        }

      if (rinfo->h->dynindx == -1 || rinfo->h->forced_local || info->symbolic)
        {
          if ((off & 1) == 0)
            {
              bfd_put_64 (output_bfd, rinfo->relocation,
                         base_got->contents + off);
              rinfo->h->got.offset |= 1;
            }
          else
            off &= ~1;
        }
  ...
  rinfo->relocation = (base_got->output_section->vma +
                      base_got->output_offset + off);
  
  bfd_reloc_status_type r = _bfd_final_link_relocate (howto, input_bfd,
                                                      input_section, contents,
                                                      rel->r_offset,
                                                      rinfo->relocation,
                                                      rel->r_addend);
  if (r != bfd_reloc_ok)
    return handle_reloc_error(r, input_bfd, input_section, rel, rinfo, howto,
                             info, status);
...
static bool
handle_ifunc_pointer(bfd *output_bfd, struct bfd_link_info *info,
                     bfd *input_bfd, asection *input_section,
                     bfd_byte *contents, Elf_Internal_Rela *rel,
                     struct reloc_info *rinfo, reloc_howto_type *howto,
                     struct elf_x86_link_hash_table *htab, bool *status)
{
  if (rel->r_addend != 0)
    {
      const char *name = get_symbol_name(rinfo->h, input_bfd, symtab_hdr,
                                         rinfo->sym);
      _bfd_error_handler
  ...
  if ((bfd_link_pic (info) && rinfo->h->non_got_ref) ||
      rinfo->h->plt.offset == (bfd_vma) -1)
    {
      return create_ifunc_dynamic_reloc(output_bfd, info, input_bfd,
                                        input_section, rel, rinfo, howto,
                                        htab);
    }
...
static bool
handle_ifunc_plt(bfd *output_bfd, struct bfd_link_info *info,
                 bfd *input_bfd, asection *input_section,
                 bfd_byte *contents, Elf_Internal_Rela *rel,
                 Elf_Internal_Rela *wrel, struct reloc_info *rinfo,
                 unsigned int r_type, reloc_howto_type *howto,
                 ...
                 unsigned int plt_entry_size, bool *status)
{
      ...
      if (htab->plt_second)
        {
          ...
          plt_offset = rinfo->eh->plt_second.offset;
        }
      else
        {
          resolved_plt = htab->elf.splt;
          plt_offset = rinfo->h->plt.offset;
        }
    }
  else
    {
  ...
  rinfo->relocation = (resolved_plt->output_section->vma +
...
static bool
handle_bad_ifunc_reloc(bfd *input_bfd, struct reloc_info *rinfo,
                       reloc_howto_type *howto, Elf_Internal_Shdr *symtab_hdr,
                       Elf_Internal_Sym *sym)
{
  const char *name = get_symbol_name(rinfo->h, input_bfd, symtab_hdr, sym);
  _bfd_error_handler
...
static bool
create_ifunc_dynamic_reloc(bfd *output_bfd, struct bfd_link_info *info,
                           bfd *input_bfd, asection *input_section,
                           Elf_Internal_Rela *rel, struct reloc_info *rinfo,
                           reloc_howto_type *howto,
                           struct elf_x86_link_hash_table *htab)
{
  ...
  if (POINTER_LOCAL_IFUNC_P (info, rinfo->h))
    {
      info->callbacks->minfo (_("Local IFUNC function `%s' in %pB\n"),
                             rinfo->h->root.root.string,
                             rinfo->h->root.u.def.section->owner);
      outrel.r_info = htab->r_info (0, R_X86_64_IRELATIVE);
      outrel.r_addend = (rinfo->h->root.u.def.value +
                        rinfo->h->root.u.def.section->output_section->vma +
                        rinfo->h->root.u.def.section->output_offset);

      if (htab->params->report_relative_reloc)
        _bfd_x86_elf_link_report_relative_reloc
          (info, input_section, rinfo->h, rinfo->sym,
           "R_X86_64_IRELATIVE", &outrel);
    }
  else
    {
      outrel.r_info = htab->r_info (rinfo->h->dynindx, r_type);
      outrel.r_addend = 0;
    }
...
static bool
handle_regular_reloc(bfd *output_bfd, struct bfd_link_info *info,
                     bfd *input_bfd, asection *input_section,
                     bfd_byte *contents, Elf_Internal_Rela *rel,
                     Elf_Internal_Rela *wrel, struct reloc_info *rinfo,
                     unsigned int r_type, reloc_howto_type *howto,
                     ...
                     unsigned int plt_entry_size, Elf_Internal_Rela *relend,
                     Elf_Internal_Shdr *symtab_hdr,
                     struct elf_link_hash_entry **sym_hashes, bool *status)
{
  bool resolved_to_zero = (rinfo->eh &&
                          UNDEFINED_WEAK_RESOLVED_TO_ZERO (info, rinfo->eh));
  ...
  switch (r_type)
    ...
    case R_X86_64_GOTPLT64:
      return handle_got_reloc(output_bfd, info, input_bfd, input_section,
                             contents, rel, wrel, rinfo, r_type, howto, htab,
                             local_got_offsets, status);

    case R_X86_64_GOTOFF64:
      return handle_gotoff_reloc(output_bfd, info, input_bfd, input_section,
                                contents, rel, wrel, rinfo, r_type, howto,
                                htab, status);

    case R_X86_64_GOTPC32:
    case R_X86_64_GOTPC64:
      return handle_gotpc_reloc(output_bfd, info, input_bfd, input_section,
                               contents, rel, wrel, rinfo, r_type, howto,
                               htab, status);

    case R_X86_64_PLTOFF64:
      return handle_pltoff_reloc(output_bfd, info, input_bfd, input_section,
                                contents, rel, wrel, rinfo, r_type, howto,
                                htab, plt_entry_size, status);

    case R_X86_64_PLT32:
      return handle_plt32_reloc(output_bfd, info, input_bfd, input_section,
                               contents, rel, wrel, rinfo, r_type, howto,
                               htab, plt_entry_size, status);

    case R_X86_64_SIZE32:
    case R_X86_64_SIZE64:
      rinfo->relocation = rinfo->st_size;
      break;
    ...
    case R_X86_64_PC32:
    case R_X86_64_PC64:
      return handle_pc_reloc(output_bfd, info, input_bfd, input_section,
                            contents, rel, wrel, rinfo, r_type, howto, htab,
                            plt_entry_size, resolved_to_zero, status);

    case R_X86_64_8:
    case R_X86_64_16:
    case R_X86_64_32:
    case R_X86_64_64:
      return handle_abs_reloc(output_bfd, info, input_bfd, input_section,
                             contents, rel, wrel, rinfo, r_type, howto, htab,
                             resolved_to_zero, status);
    ...
    case R_X86_64_CODE_5_GOTTPOFF:
    case R_X86_64_CODE_6_GOTTPOFF:
      return handle_tls_gd_ie_reloc(output_bfd, info, input_bfd, input_section,
                                    contents, rel, wrel, rinfo, r_type, howto,
                                    htab, local_got_offsets, local_tlsdesc_gotents,
                                    relend, symtab_hdr, sym_hashes, status);

    case R_X86_64_TLSLD:
      return handle_tls_ld_reloc(output_bfd, info, input_bfd, input_section,
                                contents, rel, wrel, rinfo, r_type, howto,
                                htab, relend, symtab_hdr, sym_hashes, status);

    case R_X86_64_DTPOFF32:
    case R_X86_64_TPOFF32:
    case R_X86_64_TPOFF64:
    case R_X86_64_DTPOFF64:
      return handle_tls_offset_reloc(output_bfd, info, input_bfd, input_section,
                                     contents, rel, wrel, rinfo, r_type, howto,
                                     status);
  ...
  if (rinfo->unresolved_reloc &&
      !((input_section->flags & SEC_DEBUGGING) != 0 && rinfo->h->def_dynamic) &&
      _bfd_elf_section_offset (output_bfd, info, input_section,
                              rel->r_offset) != (bfd_vma) -1)
    {
      return handle_unresolved_reloc(input_bfd, input_section, rel, rinfo,
                                     howto, info);
    }
				  ...
				  Elf_Internal_Sym *sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
    {
      if (!process_plt_entry(output_bfd, info, h, sym, htab, use_plt_second, eh, local_undefweak))
        return false;
    }
  else if (eh->plt_got.offset != (bfd_vma) -1)
    {
      if (!process_plt_got_entry(output_bfd, info, h, htab, eh))
        return false;
    }
  ...
  if (h->got.offset != (bfd_vma) -1
      ...
      && !local_undefweak)
    {
      if (!process_got_entry(output_bfd, info, h, sym, htab, eh))
        return false;
    }
                  ...
                  struct elf_x86_link_hash_entry *eh, bool local_undefweak)
{
  ...
  got_offset = calculate_got_offset(h, htab, plt);
  ...
  plt_got_pcrel_offset = calculate_plt_got_pcrel_offset(gotplt, got_offset,
                                                        resolved_plt, plt_offset, htab);
  ...
  if (!local_undefweak)
    {
      if (!fill_plt_relocation(output_bfd, info, h, sym, htab, plt, gotplt, 
                               relplt, got_offset, resolved_plt, plt_offset))
        return false;
    }
...
static bfd_vma
calculate_got_offset(struct elf_link_hash_entry *h,
                    struct elf_x86_link_hash_table *htab,
                    asection *plt)
{
  bfd_vma got_offset;
...
static bfd_vma
calculate_plt_got_pcrel_offset(asection *gotplt, bfd_vma got_offset,
                               asection *resolved_plt, bfd_vma plt_offset,
                               struct elf_x86_link_hash_table *htab)
{
  return (gotplt->output_section->vma
...
static bool
fill_plt_relocation(bfd *output_bfd, struct bfd_link_info *info,
                   struct elf_link_hash_entry *h, Elf_Internal_Sym *sym,
                   struct elf_x86_link_hash_table *htab, asection *plt,
                   asection *gotplt, asection *relplt, bfd_vma got_offset,
                   asection *resolved_plt, bfd_vma plt_offset)
{
  Elf_Internal_Rela rela;
  bfd_byte *loc;
...
static bool
process_plt_got_entry(bfd *output_bfd, struct bfd_link_info *info,
                     struct elf_link_hash_entry *h,
                     struct elf_x86_link_hash_table *htab,
                     struct elf_x86_link_hash_entry *eh)
{
                 ...
                 struct elf_x86_link_hash_entry *eh)
{
  ...
  if (h->def_regular && h->type == STT_GNU_IFUNC)
    {
      if (!process_ifunc_got_entry(output_bfd, info, h, sym, htab, eh, &rela, 
                                   &relgot, &relative_reloc_name))
        return false;
    }
...
static bool
process_ifunc_got_entry(bfd *output_bfd, struct bfd_link_info *info,
                       struct elf_link_hash_entry *h, Elf_Internal_Sym *sym,
                       struct elf_x86_link_hash_table *htab,
                       struct elf_x86_link_hash_entry *eh,
                       Elf_Internal_Rela *rela, asection **relgot,
				    ...
				    struct bfd_link_info *info)
{
  ...
  if (htab->plt.has_plt0)
    {
      if (!elf_x86_64_fill_plt0_entry(output_bfd, htab))
        return false;
    }

  if (htab->elf.tlsdesc_plt)
    {
      if (!elf_x86_64_fill_tlsdesc_plt_entry(output_bfd, htab))
        return false;
    }
...
static bool
elf_x86_64_fill_plt0_entry(bfd *output_bfd, struct elf_x86_link_hash_table *htab)
{
  bfd_vma got_plt_vma;
  bfd_vma splt_vma;
  bfd_vma got8_offset;
  bfd_vma got16_offset;

  if (htab->lazy_plt == NULL || htab->elf.splt == NULL || 
  ...
  got_plt_vma = htab->elf.sgotplt->output_section->vma
                + htab->elf.sgotplt->output_offset;
  splt_vma = htab->elf.splt->output_section->vma
             + htab->elf.splt->output_offset;

  got8_offset = got_plt_vma + 8 - splt_vma - 6;
  bfd_put_32 (output_bfd, got8_offset,
              htab->elf.splt->contents + htab->lazy_plt->plt0_got1_offset);

  got16_offset = got_plt_vma + 16 - splt_vma 
                 - htab->lazy_plt->plt0_got2_insn_end;
  bfd_put_32 (output_bfd, got16_offset,
...
static bool
elf_x86_64_fill_tlsdesc_plt_entry(bfd *output_bfd, struct elf_x86_link_hash_table *htab)
{
  bfd_vma got_plt_vma;
  bfd_vma splt_vma;
  bfd_vma sgot_vma;
  bfd_vma got8_offset;
  bfd_vma got_tdg_offset;

  if (htab->lazy_plt == NULL || htab->elf.splt == NULL || 
      htab->elf.sgotplt == NULL || htab->elf.sgot == NULL)
    return false;
  ...
  splt_vma = htab->elf.splt->output_section->vma
             + htab->elf.splt->output_offset;
  sgot_vma = htab->elf.sgot->output_section->vma
             + htab->elf.sgot->output_offset;

  got8_offset = got_plt_vma + 8 - splt_vma - htab->elf.tlsdesc_plt
                - htab->lazy_plt->plt_tlsdesc_got1_insn_end;
  bfd_put_32 (output_bfd, got8_offset,
              htab->elf.splt->contents + htab->elf.tlsdesc_plt
              + htab->lazy_plt->plt_tlsdesc_got1_offset);

  got_tdg_offset = sgot_vma + htab->elf.tlsdesc_got - splt_vma
                   - htab->elf.tlsdesc_plt
                   - htab->lazy_plt->plt_tlsdesc_got2_insn_end;
  bfd_put_32 (output_bfd, got_tdg_offset,
              htab->elf.splt->contents + htab->elf.tlsdesc_plt
  ...
  for (j = 0; plts[j].name != NULL; j++)
    {
      ...
      if (non_lazy_plt != NULL &&
          ...
          memcmp (plt_contents, non_lazy_plt->plt_entry, non_lazy_plt->plt_got_offset) == 0)
        {
...
static bool elf_x86_64_common_definition(Elf_Internal_Sym *sym)
{
  if (sym == NULL) {
    return false;
  }
			      ...
			      const bfd_target *output)
{
  ...
  if (input_backend->s == NULL || output_backend->s == NULL) {
    return false;
  }
...
elf_x86_64_add_glibc_version_dependency(struct elf_find_verdep_info *rinfo)
{
  ...
  if (rinfo->info->enable_dt_relr) {
    ...
    auto_version[version_count] = false;
    version_count++;
  }
  ...
  if (htab->params != NULL) {
    if (htab->params->gnu2_tls_version_tag && htab->has_tls_desc_call) {
      ...
      auto_version[version_count] = (htab->params->gnu2_tls_version_tag == 2);
      version_count++;
    }
  ...
  if (mark_plt_index >= 0 && !auto_version[mark_plt_index]) {
    const char *fallback_version[2] = { "GLIBC_2.36", NULL };
    bool fallback_auto[2] = { false, false };
    _bfd_elf_link_add_glibc_version_dependency(rinfo, fallback_version, fallback_auto);
  }
}
